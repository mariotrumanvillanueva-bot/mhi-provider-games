...
function doPost(e){try{
  ...
  try{data=JSON.parse(raw);}catch(parseErr){data=(e&&e.parameter)||{};}
  return json(routeAction(data,e));
...
function routeAction(data,e){
  ...
  if(a==="leaderboard")return getLeaderboard();
  if(a==="exportLeaderboard")return exportLeaderboard(data,e);
  if(a==="adminVerify")return {ok:isAdmin(data.code),version:BACKEND_VERSION};
  if(a==="ownerVerify")return {ok:isOwner(data.code),version:BACKEND_VERSION};
  if(a==="setReleaseStatus")return setReleaseStatus(data,e);
  if(a==="setWeekGame")return setWeekGame(data,e);
  if(a==="renamePlayerTypo")return renamePlayerTypo(data,e);
  if(a==="fairPlayAudit")return fairPlayAudit(data);
  if(a==="ownerAdjust")return ownerAdjust(data,e);
  if(a==="ownerResetPlayerRound")return ownerResetPlayerRound(data,e);
  if(a==="ownerArchiveResetLeaderboard")return ownerArchiveResetLeaderboard(data,e);
  if(a==="ownerDeletePlayer")return ownerDeletePlayer(data,e);
  if(a==="generateTempAdminCode")return generateTempAdminCode(data,e);
  if(a==="getActionLogs")return getActionLogs(data);
  if(a==="getSuspiciousActivity")return getSuspiciousActivity(data);
  if(a==="internalSummaryReport")return internalSummaryReport(data);
  if(a==="participationReport")return participationReport(data);
  if(a==="redemptionReport")return redemptionReport(data);
  if(a==="prizePointsReport")return prizePointsReport(data);
  if(a==="getActiveProviderRound")return getActiveProviderRound(data);
  if(a==="submitFunScore")return submitFunScore(data,e);
  if(a==="funLeaderboard")return funLeaderboard(data);
  if(a==="deleteFunUser")return deleteFunUser(data,e);
  if(a==="deepAuditReport")return deepAuditReport(data);
  if(a==="setupBonusPointsSheet")return setupBonusPointsSheet(data,e);
  if(a==="getCustomQuestions")return getCustomQuestions(data);
  if(a==="saveCustomQuestions")return saveCustomQuestions(data,e);
  if(a==="clearCustomQuestions")return clearCustomQuestions(data,e);
  if(a==="logPracticeRun")return logPracticeRun(data,e);
  if(a==="fullAuditReport")return fullAuditReport(data);
  if(a==="bonusPointReport")return bonusPointReport(data);
