(() => {
let session=null,qIndex=0,score=0,correct=0,activeQuestions=[],activeGameDef=null,selectedChecks=new Set(),answerLocked=false;const $=id=>document.getElementById(id);
...
async function joinGame(){const name=$("playerName").value.trim().replace(/\s+/g," ");const week=$("weekSelect").value;const round=$("roundSelect").value;if(!name)return alert("Enter your name first.");try{let result=await api("joinGame",{name,week,releaseType:round});if(result.requiresConfirmation){
  const proceed=confirm(result.message||"This name was recently corrected. Continue as a new player?");
  if(!proceed)return;
  const confirmResult=await api("joinGame",{name,week,releaseType:round,confirmOldTypo:true});
...
async function exportLeaderboard(code,outputId){const result=await api("exportLeaderboard",{code});if(!result.ok){if(outputId)$(outputId).textContent=JSON.stringify(result,null,2);else alert(result.message||"Export denied.");return;}downloadCSV("mhi_provider_coordinators_leaderboard.csv",result.rows);if(outputId)$(outputId).textContent="Leaderboard exported.";}
